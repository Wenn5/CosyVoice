...
class CosyVoice:
    ...
    def inference_sft(self, tts_text, spk_id, stream=False, speed=1.0,new_dropdown="无"):
        ...
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True)):
            #debug start
            if new_dropdown != "无":
                model_input = self.frontend.frontend_sft_saved(i,spk_id,new_dropdown)
                print('[DEBUG WENN]: saved: {}'.format(new_dropdown))
            else:
    ...
    def inference_instruct(self, tts_text, spk_id, instruct_text, stream=False,speed=1.0,new_dropdown="无"):
        ...
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True)):
            if new_dropdown != "无":
                model_input = self.frontend.frontend_instruct_saved(i,spk_id, instruct_text,new_dropdown)
